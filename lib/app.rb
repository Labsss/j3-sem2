...
def select_5_names(reporters)
  names = []
  5.times do
  names << reporters[rand(reporters.length)]
  end
  return names
end

def say_hello(noms, open_t)
  (noms).each do | nom |
  open_t.update("Hey #{nom}, je te souhaite une excellente journée ! On est la avec @the_hacking_pro ! #bonjour_monde")
  end
end

open_t = login_twitter
noms = select_5_names(journalistes)
say_hello(noms, open_t)